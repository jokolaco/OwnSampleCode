...
def main(training, dest, filetype='npz', frame_size=64, num_frames=30, num_images=100, original_size=28,
    ...
    if filetype == 'npz':
        ...
        for i in range(dat.shape[0]):
            if (i % num_frames == 0):
                frame_number=1
                dir_dest = save_dir + "/" + str(dir_count) 
                #print(dir_dest)
                os.mkdir(dir_dest)
                dir_count +=1

            Image.fromarray(get_image_from_array(dat, i, mean=0)).save(os.path.join(dir_dest, '{}.png'.format(frame_number)))
            frame_number+=1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dest', type=str, dest='dest', default='MMNIST_METRIC')
    ...
    parser.add_argument('--num_images', type=int, dest='num_images', default=8192)  # number of sequences to generate eval: 3072 train: 10240
